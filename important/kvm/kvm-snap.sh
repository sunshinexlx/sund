...
HELP(){
	...
	echo -e "  -m description\t\tDescription of the snapshot,Only used after '-c'"
	echo -e "  -d domain snapshot-name\tDelete snapshot of the domain"
	echo -e "  -r domain snapshot-name\tRevert domain from snapshort"
	exit
}
DOMAIN_CHECK(){
...
SNAP_CHECK(){
	virsh snapshot-list $2 |grep "\<$3\>" >/dev/null echo "Sanpshot $2 is not exist" && exit
}
case $1 in
-c)
	DOMAIN_CHECK $2
